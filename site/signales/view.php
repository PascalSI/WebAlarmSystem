		...
		<div class="portlet box <?php echo $CONFIG['WEB_THEME'];?>">
			<div class="portlet-title">
				<div class="caption">
					...
					<span id="title-signales">Últimas 100 Señales</span>
				</div>
				<div class="actions">
			...
			<div class="portlet-body">
				<div class="table-container">
					...
					<table class="table table-hover table-striped table-condensed"  id="datatablesSig">
                        <thead>
...
<script>
	...
	var btn_pdf='<button onclick="ExportPdf()" class="btn font-8 <?php echo getClassIcon('pdf')?>">';
	btn_pdf+='<i id="mini-icon" class="<?php echo getImgIcon('pdf')?>"></i></button>';

	var btn_excel='<button onclick="ExportExcel()" class="btn font-8 <?php echo getClassIcon('excel')?>">';
	btn_excel+='<i id="mini-icon" class="<?php echo getImgIcon('excel')?>"></i></button>';
	...
	view_fecha_1+="</select>&nbsp;&nbsp;"+btn_excel+"&nbsp;&nbsp;"+btn_pdf;
	...
	function initialize_table_signal(){
		oTableSig = $("#datatablesSig").dataTable({
		    ...
		    "sAjaxSource": '<?php echo $CONFIG['HOST'];?>site/signales/controller.php?acc=load_signal&fech=1&id=<?php echo $idCliente;?>&d1=&d2=&cat=-1&tipo=1',
		    ...
		    "aoColumns": [
		        {
					...
					"mRender":  function(data, type, obj) {
					    if (StrTrim(obj.ob)=="SO" || StrTrim(obj.ob)==""){
							...
							var obs = ' <a href="javascript:;" onclick="getCommentOpePanelC({id:\''+obj.id_trama+'\',evento:\''+evento+'\'})"><i>(Ver)</i></a>';
						}
						return StrTrim(obj.event)+obs;
				   }
				},
	...
	function changeDateSig(tipo){
		...
		caty_signalText=$("#select_cat option:selected").html();
		...
		oTableSig.fnReloadAjax('<?php echo $CONFIG['HOST'];?>site/signales/controller.php?acc=load_signal&fech='+date_signal+'&id=<?php echo $idCliente;?>&d1='+date1_signal+'&d2='+date2_signal+'&cat='+caty_signal+'&tipo='+auxTipo);
	}
	...
	function ExportPdf(){
		var url ='../site/signales/pdf.php?acc=load_signal&fech='+date_signal+'&id=<?php echo encode(5,$idCliente);?>&d1='+date1_signal+'&d2='+date2_signal+'&cat='+caty_signal+'&tipo='+auxTipo+"&catText="+caty_signalText;

		window.open(url,'','');
	}

	function ExportExcel(){
		var url ='../site/signales/excel.php?acc=load_signal&fech='+date_signal+'&id=<?php echo encode(5,$idCliente);?>&d1='+date1_signal+'&d2='+date2_signal+'&cat='+caty_signal+'&tipo='+auxTipo+"&catText="+caty_signalText;
